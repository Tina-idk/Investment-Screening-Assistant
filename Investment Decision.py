...
def draw_wrapped_text(c, text_str, x, y_start, max_width_chars=100, font_name="Helvetica", font_size=9, line_height=12):
    text_object = c.beginText(x, y_start)
    text_object.setFont(font_name, font_size)
    height = A4[1]

    for paragraph in text_str.split("\n"):
        lines = wrap(paragraph.strip(), width=max_width_chars)
        for line in lines:
            if text_object.getY() < 60:  # 到页底
                c.drawText(text_object)
                c.showPage()
                text_object = c.beginText(x, height - 60)
                text_object.setFont(font_name, font_size)
            text_object.textLine(line)
        text_object.textLine("")  # 段落间空行
...
def export_report_pdf(record):
    ...
    c.drawString(50, height - 50, "Investment Report")
    c.setFont("Helvetica", 10)
    c.drawString(400, height - 50, f"Date: {datetime.today().strftime('%Y-%m-%d')}")
    c.drawString(50, height - 70, f"Company File: {record['filename']}")

    # Section 1: Company Overview
    c.setFont("Helvetica-Bold", 12)
    c.drawString(50, height - 100, "1. Company Overview")
    overview_y = draw_wrapped_text(
        c,
        text_str=record["overview"],
        x=50,
        y_start=height - 115
    )
    ...
    c.setFont("Helvetica-Bold", 12)
    c.drawString(50, y, "2. AI Evaluation Scores")
    y -= 15

    scores = record["avg_score"]
    for crit, val in scores.items():
        if crit == "Total Score":
            continue
        c.setFont("Helvetica", 9)
        c.drawString(60, y, f"{crit}: {val}/5")
        y -= 12

    c.drawString(60, y - 5, f"Total Score: {scores['Total Score']} / 30")
    y -= 30
    ...
    if y < 50:
        c.showPage()
        y = height - 60
    c.drawString(50, y, "Report generated by Investment Screening Assistant")
    c.save()
    buffer.seek(0)
    return buffer

def export_comparison_pdf(records, score_df, radar_fig, summary_text):
    buffer = BytesIO()
    c = canvas.Canvas(buffer, pagesize=A4)
    width, height = A4
    ...
    # Title
    c.setFont("Helvetica-Bold", 16)
    c.drawString(50, height - 50, "Multi-Company Investment Comparison")
    c.setFont("Helvetica", 10)
    c.drawString(400, height - 50, f"Date: {datetime.today().strftime('%Y-%m-%d')}")
    ...
    c.setFont("Helvetica", 9)
    y = height - 80
    c.drawString(50, y, "Companies Compared:")
    for record in records:
        y -= 12
        c.drawString(60, y, f"- {record['filename']}")

    # Score Table Summary
    y -= 30
    c.setFont("Helvetica-Bold", 12)
    c.drawString(50, y, "1. Score Table")
    y -= 15
    c.setFont("Helvetica", 9)
    for idx, (row_idx, row) in enumerate(score_df.iterrows()):
        row_str = f"{row_idx}: " + "  ".join([f"{col}: {int(row[col])}" for col in score_df.columns])
        c.drawString(60, y, row_str[:100])  # truncate to avoid overflow
        y -= 12
        if y < 100:
            c.showPage()
            y = height - 50
    ...
    # Radar Chart
    radar_path = tempfile.NamedTemporaryFile(suffix=".png", delete=False).name
    ...
    if y < 350:
        ...
        y = height - 50
    c.setFont("Helvetica-Bold", 12)
    c.drawString(50, y, "2. Radar Chart Overview")
    c.drawImage(radar_path, x=50, y=y - 300, width=400, height=300)
    y = y - 320
    ...
    if y < 150:
        ...
        y = height - 50
    c.setFont("Helvetica-Bold", 12)
    c.drawString(50, y, "3. Gemini AI Summary")
    y -= 15
    c.setFont("Helvetica", 9)
    ...
    # Footer
    c.setFont("Helvetica-Oblique", 9)
    c.drawString(50, 40, "Report generated by Investment Screening Assistant")
    c.save()
    buffer.seek(0)
    return buffer
...
if "records" in st.session_state and len(st.session_state["records"]) > 0:
    ...
    if len(selected) >= 2:
        ...
        if st.button("📄 Export Multi-Company Comparison PDF"):
            radar_fig = plot_radar_chart(score_dict_radar)
            pdf_bytes = export_comparison_pdf(records, numeric_df.T, radar_fig, summary)
            st.download_button(
        ...
        if st.button("📄 Export Last Analyzed Company PDF"):
            record = st.session_state["records"][-1]
            pdf_bytes = export_report_pdf(record)
            st.download_button(
                label="Download Single Report PDF",
                data=pdf_bytes,
                file_name=f"{record['filename'].replace('.pdf', '')}_report.pdf",
                mime="application/pdf"
            )
